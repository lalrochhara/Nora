from Nora.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Nora import DRAGONS
...
async def Nora_is_admin(chat_id: int):
    status = False
    Nora = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Nora.id == user.id:
