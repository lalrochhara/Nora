...
import Nora.modules.sql.feds_sql as sql
from Nora import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
from Nora.modules.helper_funcs.string_handling import markdown_parser
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Nora_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Nora_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Nora_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Nora_fbanned_users.csv",
